...
class Environment:
    ...
    def assign(self, name: str, value):
        """Update an existing variable in the scope chain or create a new one in the current scope."""
        # Check if we are trying to assign to a formal parameter (Error 34)
        curr = self
        while curr:
            if name in curr.values:
                if name in curr.params:
                    raise InterpreterError(ErrorCode.SEM_COLLISION, f"Cannot assign to parameter {name}")
...
class Interpreter:
    ...
    def call_method(self, receiver, selector, args, defining_class=None, use_super=False):
        ...
        if receiver.get("class") == "Block" and defining_class is None:
            defining_class = receiver.get("val", {}).get("defining_class")

        # --- 1. Class-level messages (Constructors & Static) ---
        if receiver.get("is_class"):
            # Static check: Only from:, new, and String read are supported
            if selector == "from:":
                source_obj = args[0]
                source_val = source_obj.get("val")

                # Comtapibility control
                needs_val = (r_cls == "Integer" or self._is_subclass(r_cls, "Integer") or
                             r_cls == "String" or r_cls == "Block")

                if needs_val and source_val is None:
                    raise InterpreterError(ErrorCode.INT_INVALID_ARG, f"Source object lacks internal value for {r_cls}")
            ...
            if r_cls == "String" and selector == "read":
                ...
                if not line:  # EOF handling
                    return self._create_obj("String", "")
                return self._create_obj("String", line.rstrip('\n'))

            # If the class doesn't understand the static message
        ...
        # Type checking methods (isNumber, isString, etc.)
        type_checks = {
        ...
        if r_cls == "String":
            ...
            if selector == "length":
                ...
                # Replacing the problematic characters
            ...
            if selector == "concatenateWith:":
                # Argument should be a string or a subclass of the string
                arg_cls = args[0].get("class")
                if arg_cls == "String" or self._is_subclass(arg_cls, "String"):
                    return self._create_obj("String", str(receiver.get("val", "")) + str(args[0].get("val", "")))
                return self.nil_obj
            if selector == "startsWith:endsBefore:":
                s_val = str(receiver.get("val", ""))
                try:
                    start_idx = int(args[0].get("val", 0))
                    end_idx = int(args[1].get("val", 0))
                except (ValueError, TypeError):
                    return self.nil_obj

                # If not correct numbers return the object as NIL
                if start_idx <= 0 or end_idx <= 0:
                    return self.nil_obj

                # If the difference is null or negative, return the empty string
                if (end_idx - start_idx) <= 0:
                    return self._create_obj("String", "")

                res_str = s_val[start_idx - 1: end_idx - 1]
                return self._create_obj("String", res_str)
        ...
        # Integer specific (including subclasses)
        if r_cls == "Integer" or self._is_subclass(r_cls, "Integer"):
            ...
            # Numeric operations
            if selector in ["plus:", "minus:", "multiplyBy:", "divBy:", "greaterThan:"]:
                arg = args[0]
                if not (arg.get("class") == "Integer" or self._is_subclass(arg.get("class"), "Integer")):
                    raise InterpreterError(ErrorCode.INT_OTHER, f"Operand for {selector} must be an Integer")
                val1 = receiver.get("val", 0)
                val2 = args[0].get("val", 0)
                if selector == "plus:": return self._create_obj("Integer", val1 + val2)
                if selector == "minus:": return self._create_obj("Integer", val1 - val2)
                if selector == "multiplyBy:": return self._create_obj("Integer", val1 * val2)
                if selector == "greaterThan:":
                    res = val1 > val2
                    return self._create_obj("True" if res else "False", res)
                if selector == "divBy:":
                    if val2 == 0: raise InterpreterError(ErrorCode.INT_DNU, "Division by zero")
            ...
            if selector == "timesRepeat:":
                ...
                for i in range(1, n + 1):
                    ...
                    block_data = block_obj.get("val", {})
                    last_res = self.call_method(block_obj, "value:", [iter_num],
                                                defining_class=block_data.get('defining_class'))
                return last_res

        # Block specific
        if r_cls == "Block" and selector.startswith("value"):
            block_data = receiver["val"]
            params_count = len(block_data['node'].parameters or [])
            ...
            return self.execute_block(
                block_data['node'],
                args,
                block_data['captured_env'],
                defining_class=block_data.get('defining_class')
            )

        # Boolean and Nil specific
        if r_cls in ["True", "False"]:
            if selector == "ifTrue:ifFalse:":
                block_to_exec = args[0] if r_cls == "True" else args[1]
                block_data = block_to_exec.get("val", {})
                return self.call_method(block_to_exec, "value", [], defining_class=block_data.get('defining_class'))
            if selector == "asString":
        ...
        if use_super:
            ...
            if ctx_class and ctx_class.parent:
                ...
                # If not parent, super should not continue
        ...
        if method:
            # Create a clean local scope for the method
            method_env = Environment()
            method_env.values["self"] = receiver
            # We must pass which class we found the method in for 'super' to work correctly
        ...
        if selector.endswith(":"):
            ...
            # Collision check: Cannot create attribute if a method with the same name exists (Error 54)
            collision, _ = self._find_method(start_lookup, attr_name)
            if collision:
                raise InterpreterError(ErrorCode.INT_INST_ATTR, f"Collision: {attr_name} is a method")
    ...
    def evaluate_expr(self, expr_node, env: Environment, defining_class=None):
        ...
        if expr_node.var:
            ...
            if name == "super":
                # Creating a new disctionary but saving the references
                obj = env.lookup("self")
            ...
            # Lookup in local/parent environment variables
    ...
    def execute_block(self, block_node, args, captured_env: Environment, defining_class=None):
        ...
        # Fulfill the parameters (Error 51 check implicitly done by XML structure, but let's be safe)
        # Naplnění parametrů
