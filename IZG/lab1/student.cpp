...
void rasterizeLine(int x1, int y1, int x2, int y2, S_RGBA color)
{
    ...
    // 1. 1/2 kv. 1 a 4 - hotove, skip
    // 2. neplatna usecka
    if (x1 == x2 && y1 == y2)
        return;
    // 4. horní 1/2 kvad. 1,2,3,4
    // je potreba prvni checknout 1-4, tzn. prohodit podminku 3. a 4. z prezentace
    auto x_y_swapped = false;
    if (abs(dx) < abs(dy))
    {
        // zmena osy
        swap(x1, y1);
    ...
    for (int x = x1; x <= x2; ++x)
    {
        // 4. horni 1/2 kvadr. 1,2,3,4
        if (x_y_swapped)
        {
            // zmena souradnic pri zápisu
...
void put8PixelsOfCircle(int x, int y, int sx, int sy, S_RGBA color)
{
     ...
     */
    // pricteni vzd. ke stredu kruznice

    // [sx, sy] stred kruznice
